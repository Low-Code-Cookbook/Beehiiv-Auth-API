...
router.post('/login', (async (req, res, next) => {
  try {
    ...
    console.log(`Debug: email=${email} redirectUrl=${redirectUrl}`);

    // Do we have a valid email
    if (!email) {
      console.log('Stopped: email is missing.');
    ...
    if (!redirectUrl) {
      console.log('Stopped: redirectUrl is missing.');
      return res.status(400).json({
        success: false,
    ...
    if (!isAllowedDomain(url.hostname, allowedDomains)) {
      console.log(`Stopped: redirectUrl is not in the whitelist. ${url.hostname} in ${allowedDomains}`);
      return res.status(400).json({ 
        success: false, 
        actionCode: 'invalidRedirectUrl',
